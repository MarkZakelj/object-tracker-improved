...
parser.add_argument('--config', default=os.path.join(rootpath,'experiments/siamreppoints/config.yaml'),type=str, help='config file')
parser.add_argument('--snapshot', default=os.path.join(rootpath,'../models/siamreppoints.model'), type=str, help='tracker model name')
parser.add_argument('--armodel', default=os.path.join(rootpath,'../models/SEcmnet_ep0040.pth.tar'), type=str, help='segmentation model name')
...
class NCCTracker(object):
    ...
    def track(self, image):
        ...
        pred_bbox_rect = np.array([pred_bbox[0],pred_bbox[1],pred_bbox[0]+pred_bbox[2],pred_bbox[1]+pred_bbox[3]]).astype(np.int32)
        refined_bbox_rect = np.array([refined_bbox[0],refined_bbox[1],refined_bbox[0]+refined_bbox[2],refined_bbox[1]+refined_bbox[3]]).astype(np.int32)
        cross_box = np.array([max(pred_bbox_rect[0],refined_bbox_rect[0]),max(pred_bbox_rect[1],refined_bbox_rect[1]),
                                  min(pred_bbox_rect[2],refined_bbox_rect[2]),min(pred_bbox_rect[3],refined_bbox_rect[3])])
        mask[cross_box[1]:cross_box[3], cross_box[0]:cross_box[2]] = (output['mask'][cross_box[1]:cross_box[3], cross_box[0]:cross_box[2]] > 0.45).astype(np.uint8)
        ...
        if mask.astype(np.float).sum() / (refined_bbox[2] * refined_bbox[3]) > 0.1:
            ...
            img[int(refined_bbox[1]):int(refined_bbox[1]+refined_bbox[3]), int(refined_bbox[0]):int(refined_bbox[0]+refined_bbox[2])] = 1
            mask = img
